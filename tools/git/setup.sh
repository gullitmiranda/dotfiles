...
# Configure git accounts from the dotfiles config.yaml (variables.git)
...
dotfiles_local_dir="${dotfiles_dir}/local"
config_yaml_path="${dotfiles_dir}/config.yaml"
...
check_cmd yq "yq is not installed. You can install it using 'brew install yq'"
check_cmd git "git is not installed. Please install it using 'brew install git'"

### Execute

log info "Reading ${config_yaml_path}..."
if [[ ! -f ${config_yaml_path} ]]; then
	echo "Error: ${config_yaml_path} does not exist."
	exit 1
fi
gitconfig_data="$(yq -o=json -I=0 '.variables.git' "${config_yaml_path}")"
